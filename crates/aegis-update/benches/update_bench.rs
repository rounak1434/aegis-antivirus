...
fn main() {
    let mib: usize = std::env::args().nth(1).and_then(|a| a.parse().ok()).unwrap_or(8);
    ...
    m.signature = base64::engine::general_purpose::STANDARD.encode(sk.sign(m.signed_message().as_bytes()).to_bytes());
    ...
    println!("  download+verify    : {:.1} ms  ({:.0} MiB/s)", dl_verify.as_secs_f64() * 1000.0, mb / dl_verify.as_secs_f64());
    println!("  install            : {:.1} ms", install.as_secs_f64() * 1000.0);
    println!("  sha256 throughput  : {:.0} MiB/s", mb / hash.as_secs_f64().max(1e-6));
}
