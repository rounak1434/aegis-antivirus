...
fn signed_update_downloads_installs_and_records() {
    ...
    let m = publish(
        env.feed.path(),
        &sk,
        UpdateComponent::SignatureDatabase,
        "2024.06.22.02",
        b"sig-bundle-v2",
        "1.0.0",
    );
    ...
    assert_eq!(
        std::fs::read(&outcome.installed_path).unwrap(),
        b"sig-bundle-v2"
    );
    ...
    assert!(status
        .iter()
        .any(|(c, v)| c == "signature_database" && v == "2024.06.22.02"));
}
...
fn tampered_payload_is_rejected() {
    ...
    std::fs::write(
        env.feed.path().join("yara_rules-1.0.0.bin"),
        b"EVIL-tampered",
    )
    .unwrap();
    ...
    assert!(matches!(
        err,
        UpdateError::Verify(VerifyError::HashMismatch)
    ));
}
...
fn invalid_signature_is_rejected() {
    ...
    let mut m = publish(
        env.feed.path(),
        &sk,
        UpdateComponent::ThreatMetadata,
        "2.0.0",
        b"meta",
        "1.0.0",
    );
...
fn rollback_restores_previous_version() {
    ...
    let rolled = env
        .engine
        .rollback(UpdateComponent::SignatureDatabase)
        .unwrap();
...
fn min_app_version_blocks_install() {
    ...
    assert!(matches!(
        err,
        UpdateError::Verify(VerifyError::AppTooOld { .. })
    ));
}
