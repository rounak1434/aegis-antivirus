...
fn scan_detects_and_persists_through_service() {
    ...
    let id = orch
        .start_scan(vec![work.path().to_path_buf()], ScanMode::Full)
        .unwrap();
...
fn quarantine_restore_delete_through_service() {
    ...
    let det = threats
        .iter()
        .find(|t| t.path.contains("malware.ps1"))
        .expect("detected");
...
fn windows_scan_through_service() {
    ...
    assert!(orch
        ...
        .unwrap()
        .iter()
        .any(|t| t.path.contains("evil.exe")));
...
fn service_health_reports_components() {
    ...
    orch.add_yara_rule("t", r#"rule m { strings: $a = "x" condition: $a }"#)
        .unwrap();
...
fn realtime_start_stop_status() {
    ...
    orch.start_realtime_with_paths(
        ProtectionMode::NotifyOnly,
        vec![watch.path().display().to_string()],
    )
    .unwrap();
...
fn secure_update_flow_through_service() {
    ...
    assert_eq!(
        orch.check_updates(std::slice::from_ref(&manifest))
            .unwrap()
            .len(),
        1
    );
    ...
    assert!(status
        .iter()
        .any(|(c, v)| c == "signature_database" && v == "2024.06.22.05"));
